...
import { Check, ChevronLeft, Clock3, ShieldCheck } from "lucide-react";
...
function BookingForm(){
  ...
  return <section className="page booking-page">
    <div className="booking-heading"><div><p className="eyebrow">Réservation</p><h1>Prendre rendez-vous</h1><p className="lead">Créez votre demande en quelques étapes.</p></div>{step!=="done"&&<div className="progress-wrap" aria-label={`Étape ${progress} sur 3`}><span>Étape {progress}/3</span><div className="progress-track"><i style={{width:`${progress/3*100}%`}}/></div></div>}</div>
    ...
    <div className="booking-layout"><div className="form-card">
      ...
      {step==="details"&&<><button className="back-button" type="button" onClick={()=>setStep("service")}><ChevronLeft/> Retour</button><h2>Vos coordonnées</h2><p className="muted">Elles serviront uniquement à confirmer votre rendez-vous.</p><label>Nom et prénom<input autoComplete="name" value={name} onChange={e=>setName(e.target.value)} placeholder="Nom et prénom"/></label><label>E-mail ou téléphone<input autoComplete="email" value={contact} onChange={e=>setContact(e.target.value)} placeholder="E-mail ou téléphone"/></label><label>Note pour Erica Glow <textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Facultatif : allergies, souhait particulier…" maxLength={300}/><small>{notes.length}/300</small></label><button type="button" className="primary-button full" disabled={name.trim().length<2||contact.trim().length<5} onClick={()=>setStep("review")}>Vérifier ma demande</button></>}
