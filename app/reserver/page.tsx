...
import { Check, ChevronLeft, Clock3, Mail, MessageCircle, ShieldCheck, UserRoundCheck } from "lucide-react";
...
function BookingForm(){
  ...
  const save=async()=>{const ref=`EG-${Date.now().toString().slice(-6)}`;const draft:Draft={id:ref,service:service.name,serviceId:service.id,price:service.price,duration:service.duration,date,time,name:name.trim(),contact:contact.trim(),notes:notes.trim(),status:"Demande à confirmer",createdAt:new Date().toISOString()};const current:Draft[]=JSON.parse(localStorage.getItem("erica-glow-bookings")||"[]");localStorage.setItem("erica-glow-bookings",JSON.stringify([...current,draft]));setReference(ref);setDeliveryStatus("sending");setStep("done");try{const response=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reference:ref,name:name.trim(),email:contact.trim(),service:service.name,date,time,duration:service.duration,price:service.price,notes:notes.trim()})});setDeliveryStatus(response.ok?"sent":"error")}catch{setDeliveryStatus("error")}};
  ...
  const whatsappHref=`https://wa.me/33615461513?text=${confirmationText}`;
