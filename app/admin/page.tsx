...
export default function Page() {
    ...
    const stopAuth = onAuthStateChanged(auth, (current) => {
      ...
      stopBookings = onSnapshot(
        ...
        (snapshot) =>
          setBookings(
            snapshot.docs.map(
              (item) => ({ ...item.data(), id: item.id }) as Booking,
            ),
          ),
      ...
      stopReviews = onSnapshot(
        ...
        (snapshot) =>
          setReviews(
            snapshot.docs.map(
              (item) => ({ ...item.data(), id: item.id }) as Review,
            ),
          ),
  ...
  const confirmBooking = async (booking: Booking) => {
    ...
    try {
      ...
      setNotice({ text: `Confirmation envoyée à ${booking.contact}` });
    } catch {
      setNotice({
        text: "La confirmation n’a pas pu être envoyée. Réessayez.",
        error: true,
      });
    } finally {
      setSending("");
  ...
  const completeBooking = async (booking: Booking) => {
    ...
    try {
      ...
      setNotice({
        text: `Service terminé · invitation d’avis envoyée à ${booking.contact}`,
      });
    } catch {
      setNotice({
        text: "L’invitation d’avis n’a pas pu être envoyée. Réessayez.",
        error: true,
      });
    } finally {
      setSending("");
  ...
  if (!user)
    return (
      <section className="page admin-login">
        ...
        <h1>Espace Erica Glow</h1>
        <p className="lead">Réservé à la gestion des rendez-vous.</p>
  ...
  return (
    <section className="page admin-page">
      ...
      <div className="admin-head">
        <div>
          <p className="eyebrow">Tableau de bord privé</p>
          <h1>Réservations</h1>
          <p className="lead">
            {bookings.length} demande{bookings.length !== 1 ? "s" : ""}
          </p>
        </div>
        <button className="outline-button" onClick={() => signOut(auth)}>
          <LogOut /> Déconnexion
        </button>
      </div>
          ...
          bookings.map((item) => (
            <article key={item.id}>
              ...
              <div className="admin-meta">
                ...
                <span>
                  <Clock3 /> {item.time} · {item.duration} min
                </span>
              ...
              <div className="admin-actions">
                ...
                >
                  {sending === item.id ? "Envoi…" : "Renvoyer la confirmation"}
                </button>
                <button
                  onClick={() => completeBooking(item)}
                  disabled={sending === item.id || item.status === "completed"}
                >
                  <CheckCircle2 /> {item.status === "completed" ? "Travail terminé" : "Terminer et demander un avis"}
                </button>
              </div>
            </article>
          ))
      ...
      <div className="admin-head admin-review-head">
        <div>
        ...
        {reviews.length === 0 ? (
          ...
          reviews.map((review) => (
            <article key={review.id}>
              ...
              <div className="admin-actions">
                <button onClick={() => publishReview(review.id)}>
                  <CheckCircle2 /> Publier
                </button>
