...
export function SiteHeader(){const pathname=usePathname();return <header className="site-header"><Link className="brand-lockup" href="/" aria-label="Erica Glow — Accueil"><Image className="brand-logo" src="/icons/icon-512.png" alt="" width={58} height={58} priority/><span className="wordmark"><span>✦</span> Erica <em>Glow</em><small>LASH TECHNICIAN</small></span></Link><nav className="desktop-nav" aria-label="Navigation principale">{navItems.map(item=><Link key={item.label} className={(item.href==="/"&&pathname==="/")||(item.href==="/services"&&pathname==="/services")?"active":""} href={item.href}>{item.label}</Link>)}</nav><Link className="gold-button header-book" href="/reserver">Réserver <CalendarDays size={15}/></Link></header>}
